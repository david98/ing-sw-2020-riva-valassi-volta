  ...
  void reachablePoints(Point current) {
    for (Point point : game.getBoard().getAdjacentPositions(current)) {
      if (point.getX() < 0
        ...
        // Mi aspetto che il test fallisca in quanto viene lanciata un'eccezione teoricamente
        // scongiurata dalla getAdjacentPositions
        assertThrows(
            InvalidPositionException.class,
            () ->
                game.getCurrentPlayer()
                    .getCurrentWorker()
                    .canBePlacedOn(game.getBoard().getTopmostItem(point)));
      }
      ...
      else if (stackCurrent.peek() instanceof Block)
        currentLevel = ((Block) stackCurrent.peek()).getLevel();
      else {
        stackCurrent.pop();
        if (stackCurrent.empty()) currentLevel = 0;
        else if (stackCurrent.peek() instanceof Block)
      ...
      else if (stackPoint.peek() instanceof Block)
        pointLevel = ((Block) stackCurrent.peek()).getLevel();
      else {
        stackPoint.pop();
        if (stackPoint.empty()) pointLevel = 0;
        else if (stackPoint.peek() instanceof Block)
      ...
      try {
        ...
        // Se il punto non è lo stesso, dovrei controllare che il livello sia superiore o inferiore
        // solo di 1
        assertTrue(
            !point.equals(current)
                && (currentLevel == pointLevel || Math.abs(currentLevel - pointLevel) == 1));
