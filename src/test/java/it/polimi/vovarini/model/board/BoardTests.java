...
public class BoardTests {
  ...
  void adjacentPoints(Point p) {
      ...
      for (Point point : adjPointsList) {
        int distance =
            Math.abs(p.getX() - point.getX()) + Math.abs(p.getY() - point.getY());
        assertTrue(
            distance > 0
                && distance < 3
                && point.getX() >= 0
  ...
  @ParameterizedTest
  @MethodSource("provideInvalidPoints")
  @DisplayName("Test that place throws an InvalidPositionException when a Point is outside of the Board")
