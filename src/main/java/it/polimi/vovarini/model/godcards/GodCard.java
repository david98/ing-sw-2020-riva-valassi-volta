...
public class GodCard {
  ...
  BiFunction<Game, Point, Boolean> isPointReachable = (Game game, Point point) -> {
    try {
  ...
  BiFunction<Game, Point, Boolean> isPointBuildable = (Game game, Point point) -> {
    try {
      Worker currentWorker = game.getCurrentPlayer().getCurrentWorker();
      Point currentWorkerPosition = game.getBoard()
              .getItemPosition(currentWorker);
  ...
  Predicate<Movement> isMovementWinning = (Movement movement) -> {
  ...
  public List<Point> computeReachablePoints() throws CurrentPlayerLosesException {
    ...
    try {
      ...
      reachablePoints = candidatePositions.stream()
  ...
  public List<Point> computeBuildablePoints() throws CurrentPlayerLosesException {
    ...
    try {
      ...
      buildablePoints = candidatePositions.stream()
